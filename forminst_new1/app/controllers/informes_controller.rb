    ...
    if session[:usuario_id] && session[:tutor]
      ...
      if !@informes.blank?
        ...
        if session[:adecuacion_id]
          ...
          elsif params[:informe_id].to_i ==3
            # Si el informe es el primer anual
            puts "HELLLOOOOOO"
          ...
          elsif params[:informe_id].to_i ==6
            # Si el informe es el segundo anual
          ...
          elsif params[:informe_id].to_i ==7
            # Si el informe es el final
...
def generar_pdf() # es función permite generar el documento pdf de la adecuación
  ...
  def cambiar_estatusI
    ...
    if cpenviar == "si"
      ...
      if cpenviar=="no1"
        flash[:warning]="Debe esperar a que la adecuación sea aprobada por consejo de facultad"
